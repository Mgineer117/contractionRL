...
class HumanoidEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        obs = torch.cat(
            [
                ...
                self._commands,                                                        # (N, 3)
                self._robot.data.joint_pos - self._robot.data.default_joint_pos,      # (N, 19)
                self._robot.data.joint_vel,                                            # (N, 19)
                self._actions,                                                         # (N, 19)
            ],
            dim=-1,
        )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # penalise tilt via projected gravity (xy components should be ~0 when upright)
        rew_upright = (
            torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
            * self.cfg.rew_upright_scale
        )
        ...
        return (
            rew_lin_vel + rew_yaw + rew_z_vel + rew_ang_vel_xy
            + rew_upright + rew_torques + rew_action_rate + rew_alive
        )
